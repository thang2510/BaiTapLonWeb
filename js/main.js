...
document.addEventListener('DOMContentLoaded', function () {
    ...
    getDataFormJson("./data/data.json", function (text) {
        ...
        document.querySelector('.bkmovie__cimena').addEventListener('click', function () {
            if (document.querySelector('.bkmovie__name .dropdown__selected').innerHTML.toLowerCase() != "phim") {
                let rapchieu = '';
                document.querySelectorAll('.bkmovie__name .dropdown__menu').forEach(el => {
                    let index = el.querySelector('li.active').getAttribute('id') - 1;
                    //console.log(data.Movie[index].ID_Cinema);
        ...
        document.querySelector('.bkmovie__showtimes').addEventListener('click', function () {
            ...
            if (document.querySelector('.bkmovie__datepick .dropdown__selected').innerHTML.toLowerCase() != "ngày xem") {
                let gioxem = '';
                document.querySelectorAll('.bkmovie__name .dropdown__menu').forEach(el => {
                    let index = el.querySelector('li.active').getAttribute('id');
                    //console.log(data.Movie[index].ID_Cinema);
                ...
                dropdownSelected(this);
                // document.querySelector('.bkmovie__showtimes .dropdown__menu li').addEventListener('click', function() {
    ...
    function getBKDatePick() {
        ...
        for (i = 0; i < 7; i++) {
            ...
            if (_date == now.getDate()) {
                ...
                bkDate += '<li class="bkmovie__date"><p>Ngày mai</p><p>' + (now.getDate() + 1) + '-' + now.getMonthName() + '-' + now.getFullYear() + '</p></li>';
                //console.log('Ngày mai', now.getMonthName(), now.getFullYear());
            }
            else {
                bkDate += '<li class="bkmovie__date"><p>' + now.getDayNameNext(i, now.getUTCDay()) + '</p><p>' + (now.getDate() + i) + '-' + now.getMonthName() + '-' + now.getFullYear() + '</p></li>';
                //console.log(now.getDayNameNext(i,now.getUTCDay()), now.getMonthName(), now.getFullYear());
            }
        }
        document.querySelector('.bkmovie__datepick').addEventListener('click', function () {
            let bkDatePick = document.querySelector('.bkmovie__datepick .dropdown__menu');
    ...
    getDataFormJson("./data/movie-schedule.json", function (text) {
        ...
        for (i = 0; i < data.Cinema.length; i++) {
            if (data.Cinema[i].ID_Parent == 0) {
                if (parseInt(data.Cinema[i].ID_Cinema) == 1) {
                    cinema += '<li id="' + data.Cinema[i].ID_Cinema + '" class="nav-item mvschedule__tab__item"><a class="mvschedule__tab__link nav-link active" id="' + data.Cinema[i].CinemaLink + '-tab" ' +
                        'data-toggle="tab" href="#' + data.Cinema[i].CinemaLink + '" role="tab" aria-controls="' + data.Cinema[i].CinemaLink + '" aria-selected="true">' +
                        '<img src="./images/icons/partner/' + data.Cinema[i].CinemaLogo + '" alt="' + data.Cinema[i].CinemaName + '" /></a></li>';
                }
                else {
                    cinema += '<li id="' + data.Cinema[i].ID_Cinema + '" class="nav-item mvschedule__tab__item"><a class="mvschedule__tab__link nav-link" id="' + data.Cinema[i].CinemaLink + '-tab" ' +
                        'data-toggle="tab" href="#' + data.Cinema[i].CinemaLink + '" role="tab" aria-controls="' + data.Cinema[i].CinemaLink + '" aria-selected="true">' +
                        '<img src="./images/icons/partner/' + data.Cinema[i].CinemaLogo + '" alt="' + data.Cinema[i].CinemaName + '" /></a></li>';
                }
            }
        ...
        document.querySelectorAll('#mvschedule-tab li').forEach(el => {
            el.addEventListener('click', function() {
                document.querySelectorAll('#mvschedule-tab li').forEach(elm => {
        ...
        // Thêm tab thứ 2
        document.querySelectorAll('#mvschedule-tab').forEach(el => {
            let index = el.querySelector('li a.active').parentNode.getAttribute('id');

            let cinemaChild = '';
            cinemaChild += '<div class="row"><div class="tab-pane mvschedule__child customscroll col-12 col-sm-2 fade active show" id="' + data.Cinema[index].CinemaLink + '" role="tabpanel" aria-labelledby="' + data.Cinema[index].CinemaLink + '-tab">' +
                '<ul class="nav-tabs mvschedule__child__tab" id="mvschedule-child-tab" role="tablist">';
            for (j = 0; j < data.Cinema.length; j++) {
                if (parseInt(data.Cinema[j].ID_Parent) == parseInt(index)) {
                    cinemaChild += '<li id="' + data.Cinema[j].ID_Cinema + '" class="nav-item mvschedule__child__item"><a class="mvschedule__child__link nav-link" id="' + data.Cinema[j].CinemaLink + '-tab" ' +
                        'data-toggle="tab" href="#' + data.Cinema[j].CinemaLink + '" role="tab" aria-controls="' + data.Cinema[j].CinemaLink + '" aria-selected="true">' +
                        '<div class="image"><img src="./images/icons/movie-schedule/' + data.Cinema[j].CinemaLogo + '" alt="' + data.Cinema[j].CinemaName + '" /></div>'+
                        '<div class="content"><div class="title">'+ data.Cinema[j].CinemaName +'</div><div class="desc">' + data.Cinema[j].CinemaLocation + '</div><input class="btn-detail" type="button" value="[Chi tiết]" /></div>' +
                        '</a></li>';
                }
            }
            ...
            //console.log(document.querySelector('#mvschedule-child-tab li').firstChild);
        ...
        document.querySelectorAll('#mvschedule-tab li').forEach(el => {
            el.addEventListener('click', function() {
                ...
                document.querySelectorAll('#mvschedule-child-tab').forEach(elm => {
                    let index = elm.querySelector('li a.active').parentNode.getAttribute('id');
                    //console.log(index);
                    let cinemaMovie = '';
                    ...
                    for (j = 0; j < data.Cinema_Movie.length; j++) {
                        if (parseInt(data.Cinema_Movie[j].ID_Cinema) == parseInt(index)) {
                            cinemaMovie += '<div class="mvschedule__movie">' +
                                '<div class="image"><img src="./images/icons/movie-schedule/' + data.Cinema_Movie[j].MovieLogo + '" alt="' + data.Cinema_Movie[j].MovieName + '" /></div>' +
                                '<div class="content">' +
                                    '<div class="title"><span class="btn-age">'+ data.Cinema_Movie[j].Age +'</span>'+ data.Cinema_Movie[j].MovieName +'</div>' +
                                    '<div class="desc">'+ data.Cinema_Movie[j].MovieTime +'</div>' +
                                '</div>' +
                                '<div class="digital">'+ data.Cinema_Movie[j].Digital +'</div>' +
                                '<div class="mvschedule__showtimes">';
                ...
                document.querySelectorAll('#mvschedule-child-tab li').forEach(elm => {
                    elm.addEventListener('click', function() {
                        //console.log(el.getAttribute('id'));
                        let index = elm.getAttribute('id');
                        console.log(index);
                        let cinemaMovie = '';
                        cinemaMovie += '<div class="tab-pane fade active show" id="' + data.Cinema[index].CinemaLink + '" role="tabpanel" aria-labelledby="' + data.Cinema[index].CinemaLink + '-tab">';
                        for (j = 0; j < data.Cinema_Movie.length; j++) {
                ...
                document.querySelectorAll('#mvschedule-child-tab li').forEach(elem => {
                    elem.addEventListener('click', function() {
                        // /alert('acb');
                        document.querySelectorAll('#mvschedule-child-tab li').forEach(elme => {
        ...
        document.querySelectorAll('#mvschedule-child-tab li').forEach(el => {
            el.addEventListener('click', function() {
                // /alert('acb');
                document.querySelectorAll('#mvschedule-child-tab li').forEach(elm => {
                    elm.querySelector('a').classList.remove('active');
        ...
        document.querySelectorAll('#mvschedule-child-tab').forEach(el => {
            let index = el.querySelector('li a.active').parentNode.getAttribute('id');
            //console.log(index);
            let cinemaMovie = '';
            cinemaMovie += '<div class="tab-pane fade active show" id="' + data.Cinema[index].CinemaLink + '" role="tabpanel" aria-labelledby="' + data.Cinema[index].CinemaLink + '-tab">';
